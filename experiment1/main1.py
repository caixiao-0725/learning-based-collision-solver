...
def play(show=False,RANDOM=False):
    ...
    while not over:
        with torch.no_grad():
            action = model_action(state, env.edge_index)#+(torch.rand(6,2)-0.5)*0.01
            if RANDOM and random.random() < 0.1:
                action += (torch.rand(6,2)-0.5)*0.1
...
class Pool:
    ...
    def update(self):
        ...
        while len(pool) - old_len < 200:
            self.pool.extend(play(False,True)[0])
...
def train():
    ...
    for epoch in range(20*10):
        ...
        if epoch % 20 == 0:
            test_result = sum([play(False,False)[-1] for _ in range(20)]) / 20
